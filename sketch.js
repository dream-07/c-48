  ...
  if(score >10 && gameState=== PLAY ){
    ...
    vy +=g;
      ...
      if(keyWentDown("space")){
        bullet = createSprite(fias.x+70,fias.y-30,20,10)
        bullet.velocityX = 20
        
        bulletGroup.add(bullet)
        fias.depth = bullet.depth
        fias.depth = fias.depth+2
       // player.addImage(shooter_shooting)
       // bullets = bullets-1
      }

      if(fias.isTouching(as)&&life>0){
      ...
      if(alGroup.isTouching(bulletGroup)){
        for(var i=0;i<alGroup.length;i++){     
            
         if(alGroup[i].isTouching(bulletGroup)){
              alGroup[i].destroy()
              bulletGroup.destroyEach()
             score +=3
              } 
...
if(gameState === WON){
  ...
  fill("white")
  strokeWeight(4)
  text("WOW you won the game"+life,500,350)
  ...
  if( gameState === END){
   ...
   // gameOver.visible = true
    ...
    fill("white")
    strokeWeight(4)
    text("better luck next time"+life,500,350)
    rockGroup.destroyEach()
    starGroup.destroyEach()
    alGroup.destroyEach()
